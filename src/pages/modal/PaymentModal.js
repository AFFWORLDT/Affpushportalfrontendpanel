...
function PaymentModal({ showModal, hideModal, projects, projectInfo }) {
  return (
    <div className='modelpaymen '>
      <Modal  show={showModal} onHide={hideModal} className='model-container'>
        <Modal.Header closeButton>
          <Modal.Title className="fw-bold flex align text-primary">All Payment Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {projectInfo ? (
            <>
              {projects.map((data, index) => (
                <div key={index} className='modelpayment'>
                  ...
                  <p style={{ fontWeight: 750 }} className='fs-5 mt-2'>Beneficiary Name : <span style={{ fontWeight: 600 }}>{data.beneficiary_name}</span></p>
